...
暗电流校正又称“黑度校正”，是为了减少传感器因温度等物理和电子因素引起的电压偏移对图像质量的影响。在相机工作过程中，即使没有接收到光信号，传感器也会由于这些因素产生一定的暗电流，从而导致输出的电压信号存在偏移。为了减少这种偏移的影响，需要对传感器的暗电平进行测量，并将其从输出的电压信号中减去，从而校正这一偏移。

通常的解决方法是在传感器完全遮光的情况下捕获“暗帧”。然后记录每个像素的数字输出，计算该图像的平均值（有时使用中值）。这个平均值或中值表示传感器的整体黑电平偏移。接下来，将该值从传感器生成的图像中的每个像素值中减去，以校正暗电流引起的偏移，恢复图像的真实亮度。
